...
public class PlayerController {
    ...
    public PlayerResponse index(@PathVariable String league, @RequestParam("name") Optional<String> name) {
        return service.index(league, name);
    }

    @GetMapping(value = "/available/{league}", produces = "application/json")
    @Transactional(readOnly = true)
    public PlayerResponse available(@PathVariable String league, @RequestParam("pos") Optional<String> pos) {
